...
from captioneval.cider.pyciderevalcap.cider.cider import Cider
from captioneval.coco_caption.pycocoevalcap.bleu.bleu import Bleu
...
def LanguageModelCriterion(input, target, mask, reduction):
...
def get_self_critical_reward(greedy_res, data_gts, gen_result, args, CiderD, Cider, Bleu, res_file, label_file):


    print(greedy_res,data_gts,gen_result)
    print(type(greedy_res),type(data_gts),type(gen_result))
    #coco = COCO(label_file)
    #cocoRes = coco.loadRes(res_file)
    #cocoEval = COCOEvalCap(coco, cocoRes, 'corpus', 0, 0)
    #cocoEval.tokenize()
    ...
    print('Gen result size:',gen_result_size)
    print('Greedy res size:',greedy_res)
    ...
    greedy_res =np.array(greedy_res)
    ...
    for i in range(len(data_gts)):
        gts[i] = [array_to_str(data_gts[i]) for j in range(len(data_gts))]
    ...
    '''cond = torch.zeros(data_gts.shape[0],data_gts.shape[1]).cuda()
    data_gts = data_gts.type(torch.float32)
    data_gts = torch.where(data_gts==-1,cond.type(torch.float32),data_gts.type(torch.float32))
    data_gts = data_gts.type(torch.int32)'''
    #gts_ = data_gts.repeat(6,1).tolist()


    '''for i in range(len(gts_)):
        #DiffModel step
        gts_[i] = [array_to_str(gts_[i])]'''

    if args.cider_reward_weight > 0:
        #print('gts_:',gts_)
        #print('res_:',res_)
        print(gts_,res_)
        _, cider_scores = CiderD.compute_score(gts_, res_)
        print('Cider scores:', _)
    else:
        cider_scores = 0
    if args.bleu_reward_weight > 0:
        _, bleu_scores = Bleu.compute_score(gts_, res__)
    ...
    scores = args.cider_reward_weight * cider_scores + args.bleu_reward_weight * bleu_scores
...
class RewardCriterion(nn.Module):
    ...
    def forward(self, input, seq, reward, gt, reduction='mean'):
        
        flag = -1
        N,L = input.shape[:2]
        torch.set_printoptions(profile="full")
        #print(seq.unsqueeze(2),seq.unsqueeze(2).shape)
        try:
            input = input.gather(2, seq.unsqueeze(2)).squeeze(2)
        except:
            print('gather error:')
        ...
        #print('MASK:',mask)
        cond = torch.zeros(gt.shape[0]).cuda()
        cond2= torch.ones(gt.shape[0]).cuda()


        gt = torch.where(gt==-1,cond.type(torch.float32),cond2.type(torch.float32) )
        #print('GT.SHAPE',gt.shape,gt)
        #input = torch.where(input==50257,cond.type(torch.float32),input.type(torch.float32))
        #print('g',gt)
 
        #print('r',reward)
        output = - input * reward * gt
        #print('o',output)
        #print('Loss shapes:',input.shape,reward.shape,mask.shape)
        #print('Reduction step')
        #print(output,torch.sum(output),torch.sum(gt))
        if reduction == 'none':
            output = output.view(N,L).sum(1) / gt.view(N,L).sum(1)
        elif reduction == 'mean':
            output = torch.sum(output) / torch.sum(gt)
...
def init_scorer(cached_tokens):
    ...
    print(cached_tokens)
    CiderD_scorer = CiderD_scorer or CiderD()
    print('Type:',type(CiderD_scorer))
    global Cider_scorer
    Cider_scorer = Cider_scorer or Cider(df=cached_tokens)
    print('Type:',type(Cider_scorer))
    global Bleu_scorer
    Bleu_scorer = Bleu_scorer or Bleu(4)
    return CiderD_scorer, Cider_scorer, Bleu_scorer
...
def generate2_adpt_if(
    ...
    with torch.no_grad():
        for entry_idx in range(entry_count):
            ...
            print('Tokens sentence:',output_list)
...
def generate_diffsuion(
    ...
    with torch.no_grad():
        for entry_idx in range(entry_count):
            ...
            output_list = list(tokens.cpu().numpy())  # bs, entry_length
            print('Tokens sentence2:',output_list)
            output_text = [tokenizer.decode(_output_list) for _output_list in output_list]
            generated_list.append(output_text)
            print(generated_list)
...
def evaluate_on_coco_caption(results, res_file, label_file, xt, epoch, map_location, writer,  outfile=None):
    """
    ...
    #scoresRL = cocoEval.SC_Rewards(writer,1,0)
    #return scoresRL

    cocoEval.evaluate(writer,1,0,xt)
...
class ScstRewardCriterion(torch.nn.Module):
    ...
    def __init__(self, cider_cached_tokens='corpus', baseline_type='sample'):
    ...
    def forward(self, gt_res, greedy_res, sample_res, sample_logprobs):
        ...
        #gt_res = gt_res.tolist()
        #sample_res = sample_res.tolist()
        print('Sample logprobs:',sample_logprobs.shape)
        batch_size = len(gt_res)
        sample_res_size = len(sample_res)
        print('Batch size:',batch_size)
        print('Sample res_size',sample_res_size)
        ...
        if self.baseline_type == 'greedy':
            ...
            baseline = (sc_.sum(1, keepdims=True) - sc_) / (sc_.shape[1])
        ...
        reward = torch.as_tensor(reward, device=0, dtype=torch.float)
        print(reward)
        loss = - torch.mean(sample_logprobs,1) * reward
        loss = loss.mean()
        return loss
